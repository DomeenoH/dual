...
const getCommonPromptInstructions = (notepadContent: string, discussionMode: DiscussionMode) => {
  ...
  // Inject the specific strict JSON warning here
...
`;
  ...
  return NOTEPAD_INSTRUCTION_PROMPT_PART.replace('{notepadContent}', formatNotepadContentForAI(notepadContent)) + discussionModeInstructionText + strictJsonInstruction;
};
