...
import { DiscussionMode, ThinkingLevel } from '../types';
...
export const useSettings = () => {
  ...
  const [cognitoThinkingLevel, setCognitoThinkingLevel] = useState<ThinkingLevel>(() => {
    const validLevels = ['MINIMAL', 'LOW', 'MEDIUM', 'HIGH'];
    const stored = localStorage.getItem(COGNITO_THINKING_LEVEL_STORAGE_KEY);
    if (stored && validLevels.includes(stored)) return stored as ThinkingLevel;
    const legacy = localStorage.getItem(THINKING_LEVEL_STORAGE_KEY);
    return (legacy && validLevels.includes(legacy)) ? legacy as ThinkingLevel : 'HIGH';
  });
  ...
  const [museThinkingLevel, setMuseThinkingLevel] = useState<ThinkingLevel>(() => {
    const validLevels = ['MINIMAL', 'LOW', 'MEDIUM', 'HIGH'];
