...
import { Info, Cpu, Zap, Settings2, Ban, Gauge, Calculator, Sparkles, Box, Layout, Maximize } from 'lucide-react';
import { THINKING_BUDGET_RANGES, GEMINI_3_RO_MODELS, GEMINI_3_FLASH_MODEL_ID } from '../../constants';
...
export const ThinkingControl: React.FC<ThinkingControlProps> = ({
  ...
  const isGemini3 = GEMINI_3_RO_MODELS.includes(modelId) || modelId.includes('gemini-3-pro') || modelId.includes('gemini-3-flash');
  const isFlash3 = modelId === GEMINI_3_FLASH_MODEL_ID;
            ...
            <div className="flex justify-between items-center mb-3">
                ...
                {mode !== 'off' && (
                    <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-sky-50 text-sky-600 border border-sky-100">
                        {isFlash3 ? 'Gemini 3.0 Flash 优化' : isGemini3 ? 'Gemini 3.0 逻辑' : '思考已启用'}
                    </span>
                )}
            </div>
                        ...
                        <div className="flex flex-col gap-3">
                            <div className="flex items-center justify-between">
                                <span className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
                                    <Gauge size={12} /> 推理强度等级
                                </span>
                            </div>
                            
                            {isFlash3 ? (
                                /* 4-level Grid for Gemini 3 Flash */
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <LevelButton 
                                        active={thinkingLevel === 'MINIMAL'} 
                                        onClick={() => setThinkingLevel('MINIMAL')} 
                                        label="极简 (MINIMAL)" 
                                        desc="极速响应，适合最简单的逻辑验证。"
                                        icon={<Zap size={14} />}
                                    />
                                    <LevelButton 
                                    ...
                                    <LevelButton 
                                        ...
                                        label="高强度 (HIGH)" 
                                        desc="高强度推理，进行深度分析与复杂推理。"
                                        icon={<Cpu size={14} />}
                                    />
                                </div>
                            ) : (
                                /* Standard 2-level toggle for Pro or others */
